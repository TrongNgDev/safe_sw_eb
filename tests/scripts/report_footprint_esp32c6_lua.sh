...
ctags -R --languages=C,C++ --c-kinds=f --c++-kinds=f --fields=+n -o - $PROJECT_DIR/$project/managed_components/georgik__lua/ | cut -f1 | sort -u > $LUA_SYMBOL_FILE
...
nm -S $PROJECT_DIR/$project/build/$project.elf | grep 'image_app_lua' | grep -v 'image_app_lua_len' | awk '{printf "%d,", strtonum("0x"$2)}'
