...
/* AoT Wasm compiled from Rust */
#include "../../../applications/image_app_rust/image_app_riscv32_ilp32_aot.h"
...
iwasm_main(void *arg)
{
    ...
    uint32_t default_stack_size = 768+384;
    uint32_t host_managed_heap_size = 256;
...
app_main(void)
{
    ESP_LOGI(LOG_TAG, "Starting WAMR-AOT mode, Wasm app compiled from Rust..");
    pthread_t t;
    int res;
    ...
    printf("ESP32C6_WAMR_AOT_Rust: Load   %d\n", cpu_cycle_load);
    printf("ESP32C6_WAMR_AOT_Rust: Run    %d\n", cpu_cycle_app);
    printf("ESP32C6_WAMR_AOT_Rust: Unload %d\n", cpu_cycle_unload);
    printf("ESP32C6_WAMR_AOT_Rust: Total  %d\n", cpu_cycle_load + cpu_cycle_app + cpu_cycle_unload);
#endif
