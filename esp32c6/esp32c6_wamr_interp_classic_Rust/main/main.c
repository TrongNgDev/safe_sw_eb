...
app_main(void)
{
    ESP_LOGI(LOG_TAG, "Starting WAMR-Classic Interpreter mode, Wasm app compiled from Rust..");
    pthread_t t;
    int res;
    ...
    printf("ESP32C6_WAMR_CI_Rust: Load   %d\n", cpu_cycle_load);
    printf("ESP32C6_WAMR_CI_Rust: Run    %d\n", cpu_cycle_app);
    printf("ESP32C6_WAMR_CI_Rust: Unload %d\n", cpu_cycle_unload);
    printf("ESP32C6_WAMR_CI_Rust: Total  %d\n", cpu_cycle_load + cpu_cycle_app + cpu_cycle_unload);
#endif
