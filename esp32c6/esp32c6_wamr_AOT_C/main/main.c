...
/* AoT Wasm compiled from C */
#include "../../../applications/image_app_c/image_app_riscv32_ilp32_aot.h"
...
app_main(void)
{
    ESP_LOGI(LOG_TAG, "Starting WAMR-AOT mode, Wasm app compiled from C..");
    pthread_t t;
    int res;
    ...
    printf("ESP32C6_WAMR_AOT_C: Load   %d\n", cpu_cycle_load);
    printf("ESP32C6_WAMR_AOT_C: Run    %d\n", cpu_cycle_app);
    printf("ESP32C6_WAMR_AOT_C: Unload %d\n", cpu_cycle_unload);
    printf("ESP32C6_WAMR_AOT_C: Total  %d\n", cpu_cycle_load + cpu_cycle_app + cpu_cycle_unload);
#endif
