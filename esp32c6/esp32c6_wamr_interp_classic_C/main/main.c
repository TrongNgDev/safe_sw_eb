...
iwasm_main(void *arg)
{
    ...
    uint32_t default_stack_size = 256+384;
...
app_main(void)
{
    ESP_LOGI(LOG_TAG, "Starting WAMR-Classic Interpreter mode, Wasm app compiled from C..");
    pthread_t t;
    int res;
    ...
    printf("ESP32C6_WAMR_CI_C: Load   %d\n", cpu_cycle_load);
    printf("ESP32C6_WAMR_CI_C: Run    %d\n", cpu_cycle_app);
    printf("ESP32C6_WAMR_CI_C: Unload %d\n", cpu_cycle_unload);
    printf("ESP32C6_WAMR_CI_C: Total  %d\n", cpu_cycle_load + cpu_cycle_app + cpu_cycle_unload);
#endif
