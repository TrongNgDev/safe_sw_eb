...
app_main(void)
{
    ESP_LOGI(LOG_TAG, "Starting WAMR-Fast Interpreter mode, Wasm app compiled from C..");
    pthread_t t;
    int res;
    ...
    printf("ESP32C6_WAMR_FI_C: Load   %d\n", cpu_cycle_load);
    printf("ESP32C6_WAMR_FI_C: Run    %d\n", cpu_cycle_app);
    printf("ESP32C6_WAMR_FI_C: Unload %d\n", cpu_cycle_unload);
    printf("ESP32C6_WAMR_FI_C: Total  %d\n", cpu_cycle_load + cpu_cycle_app + cpu_cycle_unload);
#endif
